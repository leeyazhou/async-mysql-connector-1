 ...
 *
 */
public class MysqlConnection implements ChannelProcessor, AsyncConnection, InnerConnection {
  ...
  public MysqlConnection(String host, int port, String user, String password, String database, Selector selector,
      SuccessCallback onConnect) throws IOException {
    super();
    this.host = host;
    this.port = port;
    this.user = user;
  ...
  public void auth(String user, String password, String database) throws SQLException {
    if (logger.isLoggable(Level.FINE)) {
      logger.fine("user: " + user + " password: " + password + " database: " + database);
    }
    Utils.writeLong(out, MysqlDefs.CLIENT_LONG_PASSWORD | MysqlDefs.CLIENT_CONNECT_WITH_DB
        | MysqlDefs.CLIENT_PROTOCOL_41 | MysqlDefs.CLIENT_SECURE_CONNECTION | MysqlDefs.CLIENT_MULTI_RESULTS, 4);
    Utils.writeLong(out, 65536, 4);
    Utils.writeLong(out, 33, 1);
    Utils.filler(out, 23);
    Utils.nullTerminated(out, user);
    if (password.length() != 0) {
      Utils.lengthEncodedString(out, scramble411(password, handshake.getSeed()));
    } else {
      Utils.filler(out, 1);
    }
    // Utils.filler(out, 1);
  ...
  public void executeQuery(int statementId, int[] types, Object[] params) throws SQLException {
    execute(statementId, types, params);
    parser.getWaitFor().add(Protocol.RESULT_SET_PACKET_BINARY);
  }

  private void execute(int statementId, int[] types, Object[] params) throws SQLException {
    if (logger.isLoggable(Level.FINE)) {
      logger.fine("execute statement #" + statementId + " args: " + Arrays.toString(params));
    }
    ...
    System.arraycopy(nullBitsBuffer, 0, out.array(), p, nullBitsBuffer.length);
    send(0);
  }

  @Override
  public void executeUpdate(int statementId, int[] types, Object[] data) throws SQLException {
            ...
            if (result instanceof EOF) {
              if (((HasState) parser.getMessage()).isOver()) {
                if (parser.getMessage() instanceof AbstractResultSet) {
                  AbstractResultSet<?> rs = (AbstractResultSet<?>) parser.getMessage();
                  Callback callback = callbacks.remove(0);
                  if (callback != null) {
                    ((ResultSetCallback) callback).onResultSet(rs);
                  }
                }
                if (!queries.isEmpty())
              ...
              Callback callback = callbacks.remove(0);
              if (callback != null) {
                ((SuccessCallback) callback).onSuccess((OK) result);
              }
              if (!queries.isEmpty())
                key.interestOps(SelectionKey.OP_WRITE);
              ...
              if (callback != null) {
                callback.onError(new SQLException(e.getMessage(), e.getSqlState()));
              }
              if (!queries.isEmpty())
                key.interestOps(SelectionKey.OP_WRITE);
