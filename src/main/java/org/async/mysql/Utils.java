  ...
  public static void write(ByteBuffer out, int type, Object value) {
    switch (type) {
      ...
      case MysqlDefs.FIELD_TYPE_LONGLONG:
        Utils.writeLong(out, ((Number) value).longValue(), MysqlDefs.getSize(type));
        return;
      case MysqlDefs.FIELD_TYPE_FLOAT:
        Utils.writeLong(out, Float.floatToIntBits(((Number) value).floatValue()), 4);
        return;
      case MysqlDefs.FIELD_TYPE_DOUBLE:
        Utils.writeLong(out, Double.doubleToLongBits(((Number) value).doubleValue()), 8);
        return;
